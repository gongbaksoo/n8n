...
## 2026-05-16: 키워드별 클러스터링, 타임아웃 해결, excludeSources 확대

### 주요 변경 사항

#### 1. Deduplication: 채널별 → 키워드별 그룹화
- 이전: 채널별(온라인/오프라인) 그룹화 → 채널별 Top 5 = 최대 10건
- 변경: 키워드별(쿠팡/이마트/무신사...) 그룹화 → 키워드당 Top 5 = 최대 85건
- 이유: 채널은 보고용 구분일 뿐, 실제 중요한 건 키워드별 기사 확보

#### 2. Code 노드 타임아웃 해결
- 문제: AI Summary 노드가 68건 처리 시 300초 타임아웃 (N8N_RUNNERS_TASK_TIMEOUT 기본값)
- 해결: Docker 환경변수 `N8N_RUNNERS_TASK_TIMEOUT=1800` 추가 (docker-compose.yml)
- 결과: 27분 정상 실행 완료

#### 3. Gmail OAuth 프로덕션 모드 전환
- 문제: Google OAuth 앱 "Testing" 모드에서 refresh token 7일 자동 만료
- 해결: Google Cloud Console → OAuth 동의 화면 → "앱 게시" → 프로덕션 모드
- 결과: 토큰 만료 없이 영구 유지

#### 4. excludeSources 확대 (6곳 → 11곳)
- 제외 기준 수립: "2회 연속 성공 없는 언론사" 제외
  1. 이번 실행에서 실패만 있고 성공 0건인 언론사
  2. 해당 언론사가 직전 실행에서도 성공 기록 없음
- 추가된 5곳: 파이낸셜뉴스, 게임플, 바이오타임즈, 로이슈, 더스쿠프
- 최종 11곳: 뉴시스, 브릿지경제, 헤럴드경제, v.daum.net, 네이트, 아주경제, 파이낸셜뉴스, 게임플, 바이오타임즈, 로이슈, 더스쿠프

#### 5. 실행 결과 (5/16 수동 테스트)
- 수집: 1,687건 → Time Filter: 384건 → Exclusion: 261건 → Dedup: 54건
- 스크래핑 성공률: 85% (46/54건)
- 실행 시간: 21분
- Notion 등록: 54건 + 본문 블록 추가
- 이메일 발송: 정상

#### 6. Docker 재시작 후 스케줄 미작동 발견/해결
- 원인: 컨테이너 재시작 후 cron 스케줄러 재등록 안 됨
- 해결: 워크플로우 비활성화 → 재활성화

### 남은 작업
1. Slack credential 연결 및 노드 활성화
2. 워크플로우 활성화 후 자동 실행 모니터링
3. 스크래핑 실패 언론사 지속 모니터링

---

## 2026-05-17: excludeSources 추가, 스케줄 재활성화, CLAUDE.md 추가

### 주요 변경 사항

#### 1. excludeSources 추가 (11곳 → 13곳)
- 전자신문: 2회 연속 성공 없음 (5/16 1건 실패, 5/17 2건 실패)
- SR타임스: 이번 실패 + 직전 성공 기록 없음
- 최종 13곳: 뉴시스, 브릿지경제, 헤럴드경제, v.daum.net, 네이트, 아주경제, 파이낸셜뉴스, 게임플, 바이오타임즈, 로이슈, 더스쿠프, 전자신문, SR타임스

#### 2. 스케줄 트리거 미작동 재발 및 해결
- 원인: 다른 세션에서 워크플로우 수정 시 스케줄러 재등록 안 됨
- 해결: 비활성화 → 재활성화
- 교훈: API로 워크플로우를 업데이트하면 항상 deactivate → activate 필요

#### 3. CLAUDE.md 추가
- 대상 워크플로우 ID/이름/URL 명시
- 작업 규칙: 대상 워크플로우 확인 필수, 컨텍스트 신호 무시 금지

#### 4. 다른 세션 원상복구 확인
- 다른 세션에서 워크플로우를 수정했다가 원상복구한 상태 확인
- excludeSources 5곳 누락 발견 → 복원 완료

#### 5. 실행 결과 (5/17 수동 테스트)
- 수집: ~1,700건 → Dedup 후: 38건
- 스크래핑 성공률: 84% (32/38건)
- 실행 시간: 12분 32초
